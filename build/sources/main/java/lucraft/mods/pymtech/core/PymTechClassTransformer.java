...
public class PymTechClassTransformer implements IClassTransformer, Opcodes {
        ...
        for (MethodNode method : node.methods)
            if (method.name.equals("play")) {
                this.insertBefore(method.instructions, this.invoke(SOURCE_LWJGL_NAME, "checkPitch"::equals), () -> {
                    ...
                    instructions.add(new MethodInsnNode(INVOKESTATIC, "lucraft/mods/pymtech/core/SoundReverbHandler", "onPlaySound", "(I)V", false));
                    return instructions;
                });
                return true;
            }
    ...
    private byte[] applyTransform(String name, byte[] data, Predicate<ClassNode> transformer) {
        ...
        if (transformer.test(node)) {
            ...
            PymTech.LOGGER.warn("Unable to patch class {}", name);
            return data;
        }
    }
    ...
    private void insertBefore(InsnList instructions, Predicate<AbstractInsnNode> predicate, Supplier<InsnList> insert) {
        ...
        if (node != null) {
            ...
            PymTech.LOGGER.warn("Failed to find location to insert for {}", stackTrace[1].getMethodName());
        }
    }
